...
def test_slicendice1():
    ...
    cmd = [executableName]
    cmd += ['-xyzin', cexam + os.sep + 'rnase' + os.sep + 'rnase_openfold_prediction.pdb']
    cmd += ['-xyz_source', 'alphafold']
    cmd += ['-max_splits', '1']
    cmd += ['-hklin', cexam + os.sep + 'rnase' + os.sep + 'rnase25.mtz']
    cmd += ['-seqin', cexam + os.sep + 'rnase' + os.sep + 'rnase.seq']
    ...
    assert os.path.exists(outBaseName + 'split_1_refmac.mtz')  # output file was created
    assert os.path.getsize(outBaseName + 'split_1_refmac.mtz') > 0 # output file has size more than zero
    assert os.path.exists(outBaseName + 'split_1_refmac.pdb')  # output file was created
    assert os.path.getsize(outBaseName + 'split_1_refmac.pdb') > 0 # output file has size more than zero
